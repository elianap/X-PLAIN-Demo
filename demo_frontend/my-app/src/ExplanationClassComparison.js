...
function ExplanationClassComparison() {
  ...
  useEffect(() => {
    async function fetchData() {
      const res = await fetch(
        "http://127.0.0.1:5000/explanation_class_comparison"
      )
  ...
  return (
    <Container>
      <Row className="mt-3 mb-3 d-flex align-items-center">
        <Col>
          <h2>Explanation Comparison with different target classes</h2>
          <p>
            ...
            the <code> {explanation2.explainer_info.classifier_name} </code>{" "}
            classifier with respect to target class{" "}
            <code>{explanation.target_class}</code> and{" "}
            <code>{explanation2.target_class}</code> respectively. It belong to
            class <b>{explanation.target_class}</b> with probability{" "}
            <code>{explanation.prob.toFixed(3)}</code> and to{" "}
            <code>{explanation2.target_class}</code> with probability{" "}
            <code>{explanation2.prob.toFixed(3)}</code> for{" "}
...
export default ExplanationClassComparison
