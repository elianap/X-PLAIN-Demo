...
def post_classifer_2(name):
    ...
    state['explainer2'] = xp = XPLAIN_explainer(state['dataset'], state['classifier2'], random_explain_dataset=True)
    return ""
...
def get_instances():
    ...
    #state['explainer'] = xp = XPLAIN_explainer(state['dataset'], state['classifier'], random_explain_dataset=True)
    xp = state['explainer']
...
def get_explanation_comparison():
    ...
    e1 = state["last_explanation"] if state["proceed"] else xp.explain_instance(instance, target_class=class_)
...
def show_instances():
    if state['dataset'] is None or state['classifier'] is None:
        abort(400)
    if state['explainer'] is None:
        state['explainer'] = XPLAIN_explainer(state['dataset'], state['classifier'], random_explain_dataset=True)
    xp = state['explainer']
    d = xp.explain_dataset
